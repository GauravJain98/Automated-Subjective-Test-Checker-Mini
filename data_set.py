...
dataset = [
    {
        ...
        "max_marks":6,
        "data":{
            "train_ans":"Force can be defined as a push or a pull that changes or tends to change the motion of an object or changes the direction or shape of an object.  It causes objects to accelerate or add to their overall pressure. In simple terms, it is a push or a pull on an object that takes placewhen two objects interact. It is the basic cause of motion from rest. It is measured in ‘Newton’(N). Newton measures the force needed to move, accelerate or speed up objects. The basicformula for it is, product of m and a, where, ‘m’ stands for the mass in kilograms and ‘a’ stands for acceleration. It may also be measured in Pounds. To define the force acting upon an object completely it not only requires the magnitude but also information regarding thedirection of the force. A force is a vector that has a direction so to represent a force in a diagram it is common to use an arrow.",
            "test":[
                ("a force is any interaction that, when unopposed, will change the motion of an object. A force can cause an object with mass to change its velocity i.e.,to accelerate. Force can also be described intuitively as a push or a pull. A force hasboth magnitude and direction, making it a vector quantity. It is measured in the SI unit of Newton and represented by the symbol F.",2),
                ("Force can be defined as a push or a pull that changes or tends to change the state of rest or uniform motion of an object or changes the direction or shape of an object. It is measured in ‘Newton’ (N). Newton measures the force needed to move, accelerate or speed up objects. To define the force acting upon an object completely it not only requires themagnitude but also information regarding the direction of the force.",4),
                ("Force is a push or pull on an object. A force can cause an object to accelerate,slow down, remain in place, or change shape. The unit of measure for force is theNewton which is abbreviated as &quot;N&quot;. Other units of force include the dyne and thepound. Force not only has a magnitude, but it also has a direction. This makes force a vector. Vectors are shown by an arrow that indicates the direction of the force and a number that indicates the magnitude.",2.5)
            ]
        }
    }
]